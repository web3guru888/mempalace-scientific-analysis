...
from .backends import create_vector_backend
    ...
    try:
        ...
        if create:
            _collection_cache = _client_cache.get_or_create_collection(
                ...
                metadata=DRAWER_HNSW_METADATA,
            )
        elif _collection_cache is None:
            _collection_cache = _client_cache.get_collection(_config.collection_name)
        return _collection_cache
    except Exception as e:
        logger.warning("ChromaDB collection init failed: %s", e)
        return None


def _iter_all_metadatas(collection, page_size=5000):
    """Paginate through all metadatas in a collection.

    ChromaDB's ``get()`` caps at the requested ``limit``.  For large palaces
    the old ``limit=10000`` silently truncated results.  This generator pages
    through the entire collection so taxonomy / wing counts are always correct.
    """
    offset = 0
    while True:
...
def tool_status():
    ...
    try:
        ...
        logger.error("tool_status failed to iterate metadatas: %s", e)
        raise
    return {
        "total_drawers": count,
        "wings": wings,
...
def tool_list_wings():
    ...
    try:
        ...
        logger.error("tool_list_wings failed to iterate metadatas: %s", e)
        raise
    return {"wings": wings}
...
def tool_list_rooms(wing: str = None):
    ...
    try:
        if wing:
            # Use paginated get with where filter
            offset = 0
            page_size = 5000
            while True:
                batch = col.get(
        ...
        logger.error("tool_list_rooms failed to iterate metadatas: %s", e)
        raise
...
def tool_get_taxonomy():
    ...
    try:
        ...
        logger.error("tool_get_taxonomy failed to iterate metadatas: %s", e)
        raise
...
def handle_request(request):
    ...
    if method == "initialize":
        ...
        # Strip non-standard client-injected arguments (e.g. Gemini CLI
        # injects "wait_for_previous" which would cause TypeError).
