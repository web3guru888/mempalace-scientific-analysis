...
\newcommand{\stars}[2]{%
  \foreach \i in {1,...,5}{%
    \ifnum\i>#1 $\circ$\else $\star$\fi
  }%
}

% -- JMLR heading macros -----------------------------------------------------
\jmlrheading{1}{2026}{1-\pageref{LastPage}}{4/26}{}{}{Dey}
\ShortHeadings{Spatial Metaphors for LLM Memory}{Dey}
\firstpageno{1}
...
accumulated over 42{,}000 GitHub stars and claims state-of-the-art retrieval
performance on the LongMemEval benchmark (96.6\%\ Recall@5) without requiring
any LLM inference at write time.  We present a comprehensive technical
analysis of the MemPalace architecture, examining the mapping between its
cognitive-science-inspired hierarchical structure
(Wings$\to$Rooms$\to$Closets$\to$Drawers) and its actual implementation in
code.  Through independent codebase analysis, benchmark replication, and
comparison with competing systems, we find that MemPalace's headline retrieval
performance is attributable primarily to its verbatim storage philosophy
combined with ChromaDB's default embedding model (all-MiniLM-L6-v2), rather
than to its spatial organizational metaphor per se.  The palace hierarchy
operates as standard vector database metadata filtering---an effective but
well-established technique.  However, we argue that MemPalace makes several
genuinely novel contributions that the community has underappreciated: (1) a
contrarian verbatim-first storage philosophy that outperforms extraction-based
competitors, (2) an extremely low wake-up cost ($\sim$170 tokens) through its
four-layer memory stack, (3) a fully deterministic, zero-LLM write path
enabling offline operation at zero API cost, and (4) the first systematic
...
of AI memory architectures, cognitive science research on hierarchical memory,
and the emerging MCP protocol ecosystem.  Our analysis concludes that
MemPalace represents significant \emph{architectural insight} wrapped in
\emph{overstated claims}---a pattern common in rapidly adopted open-source
...
MemPalace accumulated over 7{,}000 GitHub stars.  By April 12, 2026, the
count exceeded 42{,}145 stars and 5{,}382 forks, making it one of the
fastest-growing AI projects in GitHub history.
...
A 2025 systematic review by \citet{ondrej2025} confirmed consistent
neuroimaging evidence for spatial memory circuit activation during MoL use.
...
retrieval accuracy.  A 2019 \emph{Nature} study~\citep{schapiro2019}
demonstrated that hierarchical organization emerges spontaneously in memory
for random material.  \citet{collin2020} showed the hippocampus encodes
...
tools and data sources.  MemPalace implements an MCP server with 19+ tools,
...
MemPalace (version 3.1.0, as analyzed) consists of 32 Python source files
...
\midrule
Ingestion   & \texttt{miner.py}, \texttt{convo\_miner.py}, \texttt{normalize.py}
            & Convert files/conversations to drawers \\
Storage     & \texttt{palace.py}, \texttt{backends/chroma.py}
            & ChromaDB collection management \\
Organization& \texttt{general\_extractor.py}, \texttt{room\_detector\_local.py}
            & Automatic classification into wings/rooms \\
Search      & \texttt{searcher.py}
            & Semantic similarity + metadata filtering \\
Knowledge Graph & \texttt{knowledge\_graph.py}, \texttt{entity\_registry.py}
            & SQLite entity-relationship triples \\
Compression & \texttt{dialect.py}
            & AAAK structured summarization \\
Memory Stack& \texttt{layers.py}
            & 4-layer progressive memory loading \\
Interface   & \texttt{mcp\_server.py}, \texttt{cli.py}
            & MCP tools and command-line interface \\
Maintenance & \texttt{dedup.py}, \texttt{repair.py}, \texttt{migrate.py}
...
\begin{tabular}{@{}llll@{}}
\toprule
\textbf{Palace Level} & \textbf{Code Implementation} & \textbf{ChromaDB Mapping} & \textbf{Cognitive Analog} \\
\midrule
...
Hall    & Optional metadata field & \texttt{where=\{"hall": v\}} & Corridor (optional) \\
...
standard RAG range---LangChain's default \texttt{RecursiveCharacterTextSplitter}
uses 1000/200.
...
deterministic: \texttt{drawer\_\{wing\}\_\{room\}\_\{md5(content)[:12]\}},
providing natural deduplication for identical content.
...
\midrule
L0: Identity      & User-written identity text    & $\sim$100 tokens      & Always \\
L1: Essential Story & Auto-generated from top-weight drawers & $\sim$500--800 tokens & Always \\
L2: On-Demand     & Topic/wing-specific context   & $\sim$200--500/topic  & On topic detect \\
...
The MCP server (\texttt{mcp\_server.py}, 784+ LOC) exposes 19+ tools including:
...
Hybrid v4 + Haiku rerank & 100.0\% & Yes  & LLM reranking \\
...
\begin{tabular}{@{}lllll@{}}
\toprule
\textbf{System} & \textbf{LME R@5} & \textbf{Architecture} & \textbf{Write Cost} & \textbf{Price} \\
\midrule
\textbf{MemPalace} (raw) & 96.6\% & ChromaDB + verbatim & None & Free \\
Supermemory ASMR   & $\sim$99\% & Multi-agent agentic & LLM (multi) & Paid \\
Mastra             & 94.87\%   & GPT-5-mini observational & LLM & Paid \\
Hindsight          & 91.4\%    & Retain/Recall/Reflect & LLM & Paid \\
Zep/Graphiti       & $\sim$85\% & Temporal KG (Neo4j) & LLM & \$25+/mo \\
Mem0               & $\sim$49\% & LLM fact extraction & LLM & \$19--249/mo \\
LangMem            & N/R        & LangChain primitives & LLM & Usage \\
Letta              & N/R        & 3-tier self-editing & LLM & Open/cloud \\
\bottomrule
\end{tabular}
\end{table}
...
deterministic write path.  The insight that verbatim storage plus good
embeddings can outperform LLM-mediated extraction is empirically validated
...
LangMem all extract and summarize).  The benchmark evidence supports this
philosophy: verbatim storage (96.6\%\ R@5) outperforms extraction-based
approaches (Mem0 at $\sim$49\%) by a wide margin.  The theoretical
justification is sound: extraction is a lossy operation performed under
uncertainty about future query distributions.  By deferring to retrieval-time
relevance judgment, verbatim storage avoids premature information loss.
...
However, the initial overclaiming had consequences: the project's 42{,}000+
stars were accumulated during the period of maximum claim inflation.
...
strong~\citep{collins1969,collins1975,schapiro2019,collin2020}.  But
...
collection sharding.  The v4.0.0-alpha roadmap addresses this with a backend
abstraction layer (\texttt{backends/base.py}) that could eventually support
LanceDB or PostgreSQL with pgvector.  \textbf{Knowledge graph scaling}: The
...
The project's roadmap includes: swappable backends (a \texttt{BaseCollection}
abstract class enabling LanceDB, PostgreSQL with pgvector, or community
backends); a local NLP pipeline to replace regex-based classification; hybrid
search combining semantic similarity with BM25; and security hardening.
...
($>$24{,}000 GitHub stars).  Its LongMemEval performance ($\sim$49\%) is
significantly below MemPalace's, validating the critique that extraction-based
approaches lose information.  However, Mem0 targets compact fact memory for
chatbots, and its extracted facts are more token-efficient.
...
\begin{table}[h]
\centering
\caption{MemPalace v3.1.0 codebase statistics}
\label{tab:stats}
\begin{tabular}{@{}ll@{}}
\toprule
...
GitHub stars (Apr.\ 12, 2026) & 42{,}145+ \\
GitHub forks (Apr.\ 12, 2026) & 5{,}382+ \\
